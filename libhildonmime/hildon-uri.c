...
static gboolean
uri_launch_show_animation (DBusConnection *con)
{
	DBusMessage *msg;
	...
	if (msg) {
		...
		if (dbus_message_append_args (msg,
                        ...
                        if (dbus_connection_send (con, msg, NULL) == TRUE) {
                                DEBUG_MSG (("Sent message to service: '%s'",
                                        LOADING_SCREEN_SERVICE));
                                dbus_connection_flush (con);
                                success = TRUE;
                        } else
                              DEBUG_MSG (("Couldn't send message to service: '%s'",
                                    LOADING_SCREEN_SERVICE));
			
                } else
                      DEBUG_MSG (("Couldn't append msg with service: '%s'",
                              LOADING_SCREEN_SERVICE));
		dbus_message_unref (msg);
        } else
              DEBUG_MSG (("Couldn't create msg with method: '%s' to service: '%s'",
                      LOADING_SCREEN_METHOD,
                      LOADING_SCREEN_SERVICE));
	    ...
	    GSList          *uris)
{
	...
	if (msg) {
		...
		if (dbus_connection_send (connection, msg, NULL) == TRUE) {
			...
			success = uri_launch_show_animation (connection);
		} else {
		 ...
		 GError          **error)
{
	...
	if (!action) {
		DEBUG_MSG (("URI: Attempting to open URI:'%s' using the "
			    "default since no action was given.", uri));
